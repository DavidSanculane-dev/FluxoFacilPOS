        ...
        private void SalvarDados()
        {
            ...
            using (FbConnection conn = new FbConnection(connString))
            {
                try
                {
                    ...
                    {
                        //cmd.CommandText = "INSERT INTO DEPARTAMENTO (Descricao, Observacao) VALUES (@d1, @d2)";
                        cmd.CommandText = "INSERT INTO DEPARTAMENTO (DESCRICAO, OBSERVACAO) VALUES (@d1, @d2)";
                    }
                    else // ATUALIZAR
                    {
                        cmd.CommandText = "UPDATE DEPARTAMENTO SET Descricao = @d1, Observacao = @d2 WHERE ID = @d3";
                    }
